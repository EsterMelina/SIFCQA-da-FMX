...
const AdminDashboard: React.FC = () => {
  ...
  const [presidentEditMode, setPresidentEditMode] = useState<"create" | "edit">(
    "create",
  );
  const [currentPresident, setCurrentPresident] =
    useState<PresidentData | null>(null);
          ...
          <header className={styles.topbar}>
            ...
            <div className={styles.topbarRight}>
              <button
                ...
                onClick={() =>
                  setTheme((t) =>
                    t === "light" ? "dark" : t === "dark" ? null : "light",
                  )
                }
              >
...
const DashboardContent: React.FC = () => {
  ...
  const auditLogs = [
    ...
    {
      id: 4,
      timestamp: "2024-08-10 13:55",
      user: "admin.silva",
      action: "Backup da Base de Dados",
      status: "success" as const,
    },
  ];
      ...
      <div className={styles.metricsGrid}>
        ...
        <div className={styles.metricCard}>
          ...
          <p className={styles.metricTrend}>
            <span className="material-symbols-outlined">trending_down</span> 12%
            vs última hora
          </p>
        </div>
                ...
                {auditLogs.map((log) => (
                  <tr key={log.id}>
                    ...
                    <td>
                      ...
                      >
                        <span className={styles.statusDot}></span>
                        {log.status === "success" ? "Sucesso" : "Bloqueado"}
                      </span>
                    </td>
                  </tr>
                ))}
...
const UtilizadoresSection: React.FC<{
  ...
  const handleToggleStatus = async (user: User) => {
    ...
    try {
      await http.patch(`${endpoints.users.base}/${user.id}/status`, {
        status: newStatus,
      });
